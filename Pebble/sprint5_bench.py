...
from sheets_io import (
    open_sheet, ws_by_name, read_all, rows_to_dicts, upsert_rows, now_pt_iso
)
...
def weekday_index(name: str) -> int:
    ...
    if n not in table: raise ValueError(f"Invalid day: {name}")
    return table[n]
...
def main():
    ...
    ws_over     = ws_by_name(ss, sheets.get("availability_overrides", "Availability Overrides"))
    ...
    for r in nt:
        ...
        if not nid or not main: continue
    ...
    LOG.info("Loaded %d Night Totals rows (%d unique players)", len(nt), len({m for _, m in played.keys()}))

    # --- Load Night QA (night durations and break) ---
    qa_headers, qa_rows = read_all(ws_qa)
    qa = rows_to_dicts(qa_headers, qa_rows)
    night_dur: Dict[str, Dict[str, float]] = {}  # nid -> {"pre":min,"post":min,"total":min}
    for r in qa:
        nid = (r.get("Night ID") or "").strip()
        if not nid: continue
    ...
    nights_avail_count: Dict[Tuple[str,str], bool] = {}  # (nid, main) -> any availability
    ...
    for (nid, main), p in played.items():
        ...
        if not nd:
            LOG.warning("Night QA has no durations for Night ID %s; skipping bench calc for %s", nid, main)
            continue
        night_pre  = nd["pre"]
        night_post = nd["post"]
        ...
        if default_available:
            ...
            if not avail_pre:  avail_pre  = True
            if not avail_post: avail_post = True
    ...
    ins_bn, upd_bn = upsert_rows(ws_bnight, bnh_headers, bnight_rows, ["Night ID", "Main"])
    ...
    for m, j in team_join.items():
        if j:
            join_week[m] = week_anchor_for_dt(dt_from_night_id(j, tz), tz, week_anchor_day, week_anchor_time)
        else:
            join_week[m] = None
    for m, l in team_leave.items():
        if l:
            leave_week[m] = week_anchor_for_dt(dt_from_night_id(l, tz), tz, week_anchor_day, week_anchor_time)
        else:
            leave_week[m] = None
    ...
    roster_mains = { (r.get("Main") or "").strip() for r in team if (r.get("Main") or "").strip() }
    ...
    for wk in all_weeks:
        ...
        entries: List[Tuple[str, float, float, int]] = []  # (main, week_bench, s2d_bench, avail_count)
    ...
    ins_bw, upd_bw = upsert_rows(ws_bweek, bw_headers, bweek_rows, ["Game Week (Tuesday PT)", "Main"])
    LOG.info("Bench Week Totals upserts: +%d / updated %d", ins_bw, upd_bw)

    # Service log entry
    log(ws_service, tz, "", "", "BENCH",
